...
new.cytoscape.2 <- function() {
	...
	to.edge.list <- function(mat,t, 
		...
		for (i in 1:nrow(mat)) {
			for (j in 1:ncol(mat)) {
				if (mat[i,j] > t &  row.names[[i]] != col.names[[j]]) {   #this doesnt work because rows are called like ADLPG29 and DBLPG30, need to translate first. 
					edges[[length(edges)+1]] <- c(
						n1=rownames(mat)[i], # here I will add code to check if there is a dimer event and exclude
						n1.name=row.names[[i]],
						n2=colnames(mat)[j],
						n2.name=col.names[[j]],
						weight=mat[i,j]
	...
	from.matrix <- function(mat, t, out.file="network.xgmml", 
		...
		edges <- to.edge.list(mat,t, row.names=ad.names, col.names=db.names)
		...
		#----------------------------------------------------------------------------------------------------------------------------------------
		#current plan is to filter the edges table and make a new edges table without directionality, 
		#will make a list of unique gene names in label column of grid layout output
		#with unique genes can scane the edges table and start adding them to table 
		#then remove lower scores for those that are duplicated I.e. n1 =n1 and n2=n2
		...
		edges_df<-as.data.frame(edges)
		  ...
		  for (i in 1:length(unique_list)){
		    ...
		    colnames(subset_df_2)<-c("n2", "n2.name", "n1", "n1.name", "weight", "width") #re-order DB data, so that we can just combine everything without directionality
		    undirectional_df<-rbind(undirectional_df, subset_df_1)
		    undirectional_df<-rbind(undirectional_df, subset_df_2)  
		  }
		...
		undirectional_df<-undirectional_df[with(undirectional_df, order(undirectional_df$n1.name, undirectional_df$weight)),]
		undirectional_df<-undirectional_df[!duplicated(undirectional_df$n1.name),]
		edges<-as.matrix(undirectional_df)
		#edges_df[edges_df$n1.name==as.character(unique_list[1]) | edges_df$n2.name==as.character(unique_list[1]),]
		...
		apply(nodes, 1, function(node) {
			z$addNode("node", attrs=c(label=node[["canonical"]], id=node[["id"]]), close=FALSE)#label=nodes[1,][["label"]]
