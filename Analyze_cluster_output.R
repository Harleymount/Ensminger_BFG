# Import libraries
#============================================================================================================================================
rho=0.75 # change desired rho value here
dir <- '/Users/harleymount/Desktop/bfgy2h_2018-08-16_15-38-14_CBU_Full_PRS'
setwd('/Users/harleymount/BFG_platinum_files/')
#============================================================================================================================================
...
prs.file <- getArg("PRS","res/CBU_PRS.tsv")
...
#------------Personal coes
#========================================================================================================================================================================
# Here I will try to modify the code so that I can analyze the PRS and the legionella genes seperately 
#* potential options include making seperate count files for the PRS and LPG sets 
#* instead of manually seperating them I could also do two input steps, one with barcodes only for the PRS set, and the other for LPGs and then re-merge  (NOT VIABLE REALLY)
#*counts contains 4 seperate matrices for each of the conditions tested
#new viable idea is to susbet the matrices for each data set to take out PRS and LPG samples and score seperately. 
#8 issue is findign a way to merge the two PRS and LP matrices back together they have no shared rows or columns 
#* a caveat to this approach is that I am not assessing PPI between the PRS and the LPG sets because they are split


#* another option is to perform the calculation twice with two unique RHO values and then plot values uniquely

#this approach appears to be working. What you do is run the R pipeline twice, but in one case you give it the RHO value for the PRS set (0.7 or something)
#then you run the approach again with the LPG set using the desired rho value of 0.1 (FIND OPTIMAL). 
#then you output the IPS.RData files for both rho values as csv files, then replace the PRS set with the values for the

# by doing this the PRS values dont get way over inflated, and we can keep their values within the scale of the LPG set, could even try higher rho value of 0.9 for PRS set

#uses the code below to save each IPS dataset as a csv, I then merge them in excel and reimport the csv and make it a matrix to use with the heatmap plotter: 

#---------------------code to make above work------------------------------------------
#load('ips_PRS.RData')
#write.csv(best.ips.matrix, 'PRS_set.csv')
#test<-read.csv('/Users/Harley/Desktop/hybrid_2.csv')
#row_names<-test$X
#test$X<-NULL
#rownames(test)<-row_names
#test<-as.matrix(test)
#best.ips.matrix<-test
#------------------------------------------------------------------------------------
#Load scorer tool
...
s <- mclapply(1:nrow(score.descriptor), function(i) {
	scorer$calc.s(
		counts[[ score.descriptor[[i,"selective"]] ]],  # this code goes into the counts file and finds the counts that go to either selective or permissive condition
		counts[[ score.descriptor[[i,"permissive"]] ]], # i had this set as 1 for a lot of analysis, changed back to i 
		alpha = alpha
	)
},mc.cores=n.cores)

#cs<-counts[[ score.descriptor[[1,"selective"]] ]]
#cp<-counts[[ score.descriptor[[1,"permissive"]] ]]
#.cp <- cp + 1
#sum(.cp[,1])
#sum(.cp)
#.cs <- cs + 1
...
#col<-s[[1]][,1]
#med<-median(col)
#beta<-quantile(col[col>med]-med, 0.9)
#for making correlation matrices with sprime values
#merged_sprime_1mm_exp2<-s.prime[[1]]+s.prime[[2]]
#write.csv(merged_sprime_1mm_exp2, 'merged_sprime_1mm_exp2.csv')
#save(merged_sprime_1mm_exp2, file = 'merged_sprime_1mm_exp2.Rdata')
#with both merged matrices for sprime vslaues I can calcualte correlation and log transform 
#plot_dataframe<-data.frame(exp1=log(c(merged_sprime_1mm_exp1[,]), base=2),exp2=log(c(merged_sprime_1mm_exp2[,]), base=2), stringsAsFactors=F)
#library(corrgram)
#corrgram(cor(merged_sprime_1mm_exp1,merged_sprime_1mm_exp2))
#cor.test(merged_sprime_1mm_exp1,merged_sprime_1mm_exp2)
#cor.test(merged_sprime_1mm_exp1[1:54,1:49],merged_sprime_1mm_exp2[1:54,1:49]) # JUST PRS
#library(ggplot2)
#ggplot(data=plot_dataframe)+geom_point(x=exp1, y=exp2) # make dotplot
...
#HARLEYS NOTES: SEEMS LIKE THE SPRIME TABLE NEEDS TO BE MANIPULATED TO FIND NOISE IN PROTEIN PAIRS, THIS HAS ALL THE CLONE INFO BEFORE COMBINING INTO ONE NUMBER
...
genewise.scores <- mclapply(1:nrow(genewise.descriptor), function(i) {
	...
	mat.in <- s.prime[[which( #=================================================NOTE I CHANGED THIS LINE TO S.PRIME_PRS FROM S.PRIME 
...
#write.table(best.ips.matrix, file='best_Matrix_maybe.txt')
###
# Obtain gold standards
#
gs <- new.goldstandards()
logger$info("Loading Legionella PRS")
...
for(i in 1:length(ips.scores)){
  ips.scores[[i]][is.na(ips.scores[[i]])]<-min(ips.scores[[i]][!(is.na(ips.scores[[i]]))])
}

mccs <- lapply(list(hi3=hi3.matrix,prs=prs.matrix,union=unionGS.matrix), function(gold.matrix) {
  #gold.matrix<-prs.matrix #for debugging
	do.call(rbind,mclapply(1:nrow(parameter.space), function(i) {
		...
		#mcc(-0.3956954,pairs[,"score"], pairs[,"gold"] ) #
		idx <- which.max(mccs[,"mcc"])
		c(best.t=ts[idx],mccs[idx,])
		#c(best.t=ts[2],mccs$union[2,])
	},mc.cores=n.cores))

})


#Pick the parameters with the best MCC in the Legionella set. If you want the set that has best for both human and legionella change prs to union

best.param <- which.max(mccs$prs[,"mcc"]) # change prs to union for best overlap of both human and LP sets
#And pick the corresponding IPS matrix.
best.ips.matrix <- ips.scores[[best.param]]

#write.csv(best.ips.matrix,'LPG_matrix.csv')
#personal code
#---------------------------
#============== I added this stuff
#prs_df<-read.csv('hybridmatrix.csv')
#row_names<-prs_df$X
#prs_df$X<-NULL

#prs_matrix<-as.matrix(prs_df)
#rownames(prs_matrix)<-row_names
#hybridmatrix<-prs_matrix


#lpg_df<-read.csv('LPG_matrix.csv')
#row_names<-lpg_df$X
#lpg_df$X<-NULL

#lpg_matrix<-as.matrix(lpg_df)
#rownames(lpg_matrix)<-row_names
...
save(scores.3d, file=paste(dir,paste0("s-prime_",rho,".rdata"),sep=""))
save(best.ips.matrix, file=paste(dir,paste0("ips_",rho,".rdata"),sep=""))
###
#Draw MCCs plots
#
...
png(paste(dir,paste0("MCCs_",rho,".png"),sep=""),400*mfrow[2],300*mfrow[1])
...
#png(paste(dir,paste0("ips_barplot_",rho,".png"),sep=""),1000,500)
postscript(paste(dir,paste0("ips_barplot_",rho,".ps"),sep=""), width=4000, height=500)
layout(matrix(1:3,ncol=1),heights=c(1,.25,.5))

op <- par(mar=c(0,4,4,1)+.1,las=3,cex=.7)#changed cex from .7 to .4 here 
...
axis(2,at=1:3-.5,labels=c("Union","HI3","LP PRS"),tick=FALSE)
...
op <- par(mar=c(0,4,0,1)+.1,cex=.7) # changed cex here from .7 to .4 
...
#Rank oprdered lists 
#------------------------------
#---outputting barplot data for rank ordered lists 
ad.sym <- sapply(ips.table$AD,function(x)bc2gene[[x]])
db.sym <- sapply(ips.table$DB,function(x)bc2gene[[x]])
ips.table$AD<-ad.sym
ips.table$DB<-db.sym
outfile <- paste(dir,paste0("Rank_ordered_data_", rho,".csv"),sep="")
write.csv(ips.table, outfile)
...
yp <- new.yogi.plotter()
#png(paste(dir,paste0("ips_heatmap_",rho,".png"),sep=""),1000,1000)
postscript(paste(dir,paste0("ips_heatmap_",rho,".ps"),sep=""), width=20, height=20)
...
#=============================================================================
#LOG transformation to make heatmaps look better
best.ips.matrix_log<-best.ips.matrix+abs(min(best.ips.matrix))+10
best.ips.matrix_log<-log2(best.ips.matrix_log)
logger$info("Drawing IPS heatmap")
yp <- new.yogi.plotter()
#png(paste(dir,paste0("ips_heatmap_",rho,".png"),sep=""),1000,1000)
postscript(paste(dir,paste0("ips_heatmap_",rho,"_log2.ps"),sep=""), width=20, height=20)
...
#PRS heatmap
#------------------------------------------------
#makign just the PRS matrix from a large dataset
prs_matrix<-best.ips.matrix[1:16,1:17]
logger$info("Drawing IPS heatmap")
yp <- new.yogi.plotter()

postscript(paste(dir,"ips_heatmap_PRS_", rho,".ps",sep=""), width=20, height=20)
#png(paste(dir,"ips_heatmap_PRS_", rho,".png",sep=""),1000,1000)
ad.names <- sapply(rownames(prs_matrix),function(name) with(barcodes,unique(symbol[id==name])))
db.names <- sapply(colnames(prs_matrix),function(name) with(barcodes,unique(symbol[id==name])))
ad.prs.idx <- max(which(sapply(rownames(prs_matrix),function(name) with(barcodes,unique(type[id==name])))=="PRS"))
db.prs.idx <- max(which(sapply(colnames(prs_matrix),function(name) with(barcodes,unique(type[id==name])))=="PRS"))
...
source('lib/cytoscape3_undirectional.R')
...
cy$from.matrix(
	...
	out.file=paste(dir,paste0("cytoscape_",rho,".xgmml"),sep=""),
	row.names=ad.names,
	col.names=db.names
)
...
if (!file.exists(outfile)) {
	...
	if (length(raw.nums) > 0) {
		...
		pie(error.percent,labels=labels,clockwise=TRUE,col=grey.colors(length(error.percent)),cex=.4)
		par(op)
		invisible(dev.off())
...
read_quality<-as.data.frame(raw.nums)
read_quality<-t(read_quality)
read_quality<-as.data.frame(read_quality)
colnames(read_quality)<-'raw_values'
outfile <- paste(dir,"read_alignment_data.csv",sep="")
write.csv(read_quality, outfile)